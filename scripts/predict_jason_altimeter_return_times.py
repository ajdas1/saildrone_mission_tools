...
for fl in fls:
    with xr.open_dataset(fl, engine="netcdf4") as ds:
        ...
        tmp = tmp[
            (tmp.longitude >= point_lon - 1.0) & (tmp.longitude <= point_lon + 1.0)
        ]
        tmp = tmp[(tmp.latitude >= point_lat - 1.0) & (tmp.latitude <= point_lat + 1.0)]
        data.append(tmp)

data = pd.concat(data, axis=0).reset_index(drop=True)
data["dt_days"] = (
    (data.time - data.time.iloc[0]).dt.total_seconds() / 60 / 60 / 24
).astype(int)
data["dist_point"] = data.apply(
    lambda x: great_circle_distance(x.longitude, x.latitude, point_lon, point_lat),
    axis=1,
)
...
data_return = data.loc[data.groupby("dt_days").dist_point.idxmin()].reset_index(
    drop=True
)
data_return = data_return[data_return.dist_point <= 10]
data_return = data_return[["time", "latitude", "longitude", "dist_point"]]
avg_return_seconds = (
    ((data_return.time - data_return.time.iloc[0]).dt.total_seconds()).diff().mean()
)
