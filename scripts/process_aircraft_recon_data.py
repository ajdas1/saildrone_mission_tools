...
from read_file import (
    combine_aircraft_recon_drop,
    combine_aircraft_recon_hdob,
    interpret_dropsonde_text_file,
)
...
for flight in knhc_flight_unique:
    ...
    flight_data_ids = combine_aircraft_recon_hdob(
        ...
        flight=flight,
        flight_list=knhc_flight,
        recon_dir=recon_flight_dir,
        center=center,
    )
    for idx in flight_data_ids:
        os.remove(f"{recon_flight_dir}{os.sep}{recon_fls_knhc[idx]}")

    drop_data_ids, drop_data = combine_aircraft_recon_drop(
        data=knhc_drop_data,
        flight=flight,
        flight_list=knhc_drop,
        recon_dir=recon_drop_dir,
        center=center,
    )

    if len(drop_data) > 0:
        flight_date = datetime.strptime(
            recon_drops_knhc[drop_data_ids[0]].split(".")[1], "%Y%m%d%H%M"
        )
...
for flight in kwbc_flight_unique:
    ...
    if len(drop_data) > 0:
        flight_date = datetime.strptime(
            recon_drops_kwbc[drop_data_ids[0]].split(".")[1], "%Y%m%d%H%M"
        )
        filename = f"{recon_drop_dir}{os.sep}{center}_{flight_date.strftime('%Y%m%d')}_{flight.replace(' ', '-')}.csv"
        drop_data.to_csv(filename, index=False)
        for idx in drop_data_ids:
