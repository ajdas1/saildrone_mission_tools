...
from atcf_processing import column_names, column_types, fix_atcf_latitude, fix_atcf_longitude, get_atcf_files
...
if config["download_nhc_atcf_data"]:
    for fl in os.listdir(adecks_dir):
        if "aal" in fl:
            df = pd.read_csv(f"{adecks_dir}{os.sep}{fl}", header=None, names=column_names, dtype=column_types, delimiter=",", index_col=False, skipinitialspace=True, na_values=["Q"])
            df.Date =  pd.to_datetime(df.Date, format="%Y%m%d%H")
            df.Latitude = df.Latitude.apply(fix_atcf_latitude)
            df.Longitude = df.Longitude.apply(fix_atcf_longitude)
            df.to_csv(f"{adecks_dir}{os.sep}{fl[1:3]}_{fl[5:9]}-{fl[3:5]}.dat", index=False)
            os.remove(f"{adecks_dir}{os.sep}{fl}")

    for fl in os.listdir(bdecks_dir):
        if "bal" in fl:
            df = pd.read_csv(f"{bdecks_dir}{os.sep}{fl}", header=None, names=column_names, dtype=column_types, delimiter=",", index_col=False, skipinitialspace=True)
            df.Date =  pd.to_datetime(df.Date, format="%Y%m%d%H")
            df.Latitude = df.Latitude.apply(fix_atcf_latitude)
            df.Longitude = df.Longitude.apply(fix_atcf_longitude)
            df.to_csv(f"{bdecks_dir}{os.sep}{fl[1:3]}_{fl[5:9]}-{fl[3:5]}.dat", index=False)
            os.remove(f"{bdecks_dir}{os.sep}{fl}")


if preprocess_wind_radii:
    adecks_dir_windradii = f"{f'{os.sep}'.join(adecks_dir.split(os.sep)[:-1])}{os.sep}" + f"wind_radii"
    bdecks_dir_windradii = f"{f'{os.sep}'.join(bdecks_dir.split(os.sep)[:-1])}{os.sep}" + f"wind_radii"
    ...
    for fl in fls:
        ...
        df = df.drop(columns=[
            ...
            "SeasRadius3", "SeasRadius4", "x", "xx", "xxx", "xxxx", "xxxxx", "xxxxxx", "xxxxxxx", "xxxxxxxx", "xxxxxxxxx", "y", "yy"
            ])
        ...
        for wr in wrs:
            if (not pd.isna(wr)) and (wr > 0):
                ...
                if len(df_sub) > 0:
                    df_sub.to_csv(f"{adecks_dir_windradii}{os.sep}{fl[:-4]}_{wr}kt.dat", index=False)
        ...
        for wr in wrs:
            if (not pd.isna(wr)) and (wr > 0):
                ...
                df_sub = df_sub.drop(columns=["WindIntensityForRadii"])
                if len(df_sub) > 0:
                    df_sub.to_csv(f"{bdecks_dir_windradii}{os.sep}{fl[:-4]}_{wr}kt.dat", index=False)
