...
current_time = convert_time_to_utc(time=datetime.now(), timezone = pytz.timezone("US/Eastern"))
fcst_time_last = current_time.replace(hour=current_time.hour//6*6, minute=0, second=0, microsecond=0, tzinfo=None)
...
for fl in btk_fls:
    ...
    df["Valid"] = df.Date
    df = df[["StormName", "StormNumber", "Valid", "FcstCenter", "Latitude", "Longitude", "MaxSustainedWind", "MinSLP"]]
