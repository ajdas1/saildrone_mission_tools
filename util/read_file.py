...
def read_dropsonde_part_a_pgtw(data: List[str], drop_dict: dict) -> dict:
        ...
        pressure_indices = {idx: aidx for idx in part_a_pressure_indicators for aidx, val in enumerate(data) if val[:2] == idx}
        ...
        for lev in part_a_pressure_indicators:
            if lev in pressure_indices:
                ...
                _, p_level, geo = decode_pressure_geopotential(value=data[pressure_indices[lev]], part="a")
...
def read_dropsonde_part_a_position(data: List[str], drop_dict: dict, drop_date: datetime) -> dict:
    ...
    if len(data) == 3:
        if data[0] == "SPG":
            ...
            drop_dict["end_time"] = [drop_date.replace(hour=int(data[2][:2]), minute=int(data[2][2:4]), second=int(data[2][4:6]))]
            ...
            drop_dict["start_time"] = [drop_date.replace(hour=int(data[2][:2]), minute=int(data[2][2:4]), second=int(data[2][4:6]))]
        ...
        drop_dict["start_time"] = [drop_date.replace(hour=int(data[2][:2]), minute=int(data[2][2:4]), second=int(data[2][4:6]))]
        drop_dict["end_time"] = [drop_date.replace(hour=int(data[5][:2]), minute=int(data[5][2:4]), second=int(data[5][4:6]))]
        drop_lat, drop_lon = decode_drop_position(data[1])
        drop_dict["start_lon"] = [drop_lon]
        drop_dict["start_lat"] = [drop_lat]
...
def read_dropsonde_part_b_pt(data: List[str], drop_dict: dict) -> dict:
    ...
    pressure_indices = {idx: aidx for idx in part_b_pressure_indicators for aidx, val in enumerate(data) if val[:2] == idx}
    ...
    for lev in part_b_pressure_indicators:
        if lev in pressure_indices:
            ...
            _, p_level, geo = decode_pressure_geopotential(value=data[pressure_indices[lev]], part="b")
            pressure.append(p_level)
            geopotential.append(geo)
            try:
                temp, dwpt = decode_temperature(value=data[pressure_indices[lev]+1])
...
def interpret_dropsonde_text_file(data: List[str], filename: str) -> pd.DataFrame:
    ...
    section_idx = {sec: idx for sec in data_a_sections for idx, line in enumerate(data_a) if line[:5]==sec}
    ...
    drop_dict = {"pressure": [], "geopotential": [], "temperature": [], "dewpoint": [], "u": [], "v": []}
    ...
    drop_dict = read_dropsonde_part_a_position(data=data_a_pos, drop_dict=drop_dict, drop_date=drop_date)



    data_b_sections = ["XXBB ", "21212", "31313", "61616", "62626"]
    section_idx = {sec: idx for sec in data_b_sections for idx, line in enumerate(data_b) if line[:5]==sec}
    ...
    drop_data = pd.DataFrame([], columns=list(drop_dict.keys()), index=range(len(drop_dict["pressure"])))
    ...
    drop_data = drop_data.sort_values(by="pressure", ascending=False).reset_index(drop=True)
...
def combine_aircraft_recon_drop(data: List[str], flight: str, flight_list: List[str], recon_dir: str, center: str = "KNHC") -> List:
...
def combine_aircraft_recon_hdob(data: List[str], flight: str, flight_list: List[str], recon_dir: str, center: str = "KNHC") -> List:
    data_idx = [idx for idx, fl in enumerate(flight_list) if fl == flight]
    flight_data = [dat for idx, dat in enumerate(data) if idx in data_idx]
    flight_date = [datetime.strptime(dat[2].split(" ")[-1], "%Y%m%d") for dat in flight_data]
    flight_data = [dat[3:] for dat in flight_data]
    flight_data = [read_aircraft_recon_hdob_file(dat, flight_date=flight_date[idx]).round(2) for idx, dat in enumerate(flight_data)]
...
def read_aircraft_recon_hdob_file(data: List[str], flight_date: datetime) -> pd.DataFrame:


    data = [line.rstrip().split(" ") for line in data]
    time = [flight_date.replace(hour=int(line[0][:2]), minute=int(line[0][2:4]), second=int(line[0][4:6]), microsecond=0) for line in data]
